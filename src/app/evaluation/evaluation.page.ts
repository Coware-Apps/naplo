...
export class EvaluationPage implements OnInit, OnDestroy {
  ...
  async ionViewWillEnter() {
    ...
    for (let i = 0; i < this.napToCheck; i++) {
      ...
      (await this.kreta.getTimetable(d))
        .pipe(takeUntil(componentDestroyed(this)))
        .subscribe(
          x => {
            x.forEach(ora => {
            ...
            if (i == this.napToCheck - 1) {
              this.csoportok.sort((a, b) => a.OsztalyCsoportNev.localeCompare(b.OsztalyCsoportNev));
              this.loading = false;
          ...
          e => {
            if (this.config.debugging)
              this.errorHelper.presentAlert(e);
          }
        );
    }
  ...
  async onCsoportClick(c: TanitottCsoport) {
    ...
    if (this.networkStatus.getCurrentNetworkStatus() === ConnectionStatus.Offline)
      return await this.errorHelper.presentToast("Nincs internetkapcsolat, ezért a csoport most nem értékelhető!");
