...
export class KretaService {
  ...
  public get institute(): Institute { return this._institute; }
  ...
  public get currentUser(): Jwt { return this._currentUser; }
  public set currentUser(v: Jwt) { this._currentUser = v; }
  ...
  public async getValidAccessToken(): Promise<string> {
    try {
      ...
      const access_token = await this.data.getItem<string>("access_token")
        .catch(() => {
      ...
      const refresh_token = await this.data.getItem<string>("refresh_token")
        .catch(() => {
  ...
  async loginWithUsername(username: string, password: string): Promise<HTTPResponse> {
    try {
      ...
      const response = await this.data.postUrl(this.idpUrl + "/connect/Token", {
  ...
  async getInstituteList(): Promise<Observable<Institute[]>> {
    return (await this.data.getUrlWithCache('https://kretaglobalmobileapi.ekreta.hu/api/v1/Institute', null, {
  ...
  async getAuthenticatedAdatcsomag<T>(url: string, cacheSecs: number = 30 * 60, forceRefresh: boolean = false): Promise<Observable<T>> {
    const access_token = await this.getValidAccessToken();
    return (await this.data.getUrlWithCache(this.institute.Url + url, null, {
      'Authorization': 'Bearer ' + access_token,
    },
      cacheSecs, forceRefresh))
      .pipe(
        map(x => JSON.parse(x.data)),
        map(x => x.Adatcsomag),
      );
  ...
  async getTanarProfil(): Promise<Observable<TanarProfil>> {
    ...
    return this.getAuthenticatedAdatcsomag<TanarProfil>("/Naplo/v2/Tanar/Profil", Number.MAX_VALUE);
  }

  async getNaploEnum(engedelyezettEnumName: string = "MulasztasTipusEnum"): Promise<KretaEnum[]> {
    const access_token = await this.getValidAccessToken();
    return (await this.data.getUrlWithCache(this.institute.Url + "/Naplo/v2/Enum/NaploEnum?hash=&engedelyezettEnumName=" + engedelyezettEnumName, null, {
      'Authorization': 'Bearer ' + access_token,
    },
      Number.MAX_VALUE))
      .pipe(
  ...
  async getTimetable(day: Date, forceRefresh: boolean = false): Promise<Observable<Lesson[]>> {
    ...
    return this.getAuthenticatedAdatcsomag<Lesson[]>("/Naplo/v2/Orarend/OraLista?datumUtc=" + day.toISOString(), 30 * 60, forceRefresh);
  }

  async getOsztalyTanuloi(osztalyCsoportId: number): Promise<Observable<OsztalyTanuloi>> {
    console.log("getOsztalyTanuloi()");

    return this.getAuthenticatedAdatcsomag<OsztalyTanuloi>("/Naplo/v2/Ora/OsztalyTanuloi?osztalyCsoportId=" + osztalyCsoportId, 60 * 60 * 24 * 3);
  }
  ...
  async getJavasoltJelenlet(ora: Lesson): Promise<Observable<JavasoltJelenletTemplate>> {
    ...
    if (ora.OrarendiOraId)
      url = this.institute.Url + "/Naplo/v2/Ora/OrarendiOra/JavasoltJelenlet?key[0].OrarendiOraId=" + ora.OrarendiOraId
        + "&key[0].OraKezdetDatumaUtc=" + ora.KezdeteUtc
        + "&key[0].OraVegDatumaUtc=" + ora.VegeUtc;
    else
      url = this.institute.Url + "/Naplo/v2/Ora/TanitasiOra/JavasoltJelenlet?key[0].TanitasiOraId=" + ora.TanitasiOraId;

    const access_token = await this.getValidAccessToken();
    return (await this.data.getUrlWithCache(url, null, {
      'Authorization': 'Bearer ' + access_token,
    }))
      .pipe(
        map(x => JSON.parse(x.data)),
  ...
  async getMulasztas(tanoraid: number): Promise<Observable<Mulasztas[]>> {
    ...
    return this.getAuthenticatedAdatcsomag("/Naplo/v2/Ora/Mulasztas?hash=&tanoraId=" + tanoraid);
  }

  async getFeljegyzes(tanoraid: number): Promise<Observable<Feljegyzes[]>> {
    console.log("getFeljegyzes()");

    return this.getAuthenticatedAdatcsomag("/Naplo/v2/Ora/Feljegyzes?hash=&tanoraId=" + tanoraid);
  }
  ...
  async postLesson(data: object): Promise<any> {
    const access_token = await this.getValidAccessToken();
    const response = await this.data.postUrl(this.institute.Url + "/Naplo/v2/Orarend/OraNaplozas", data, {
      'Authorization': 'Bearer ' + access_token,
    }, "json");
  ...
  async postErtekeles(data: object): Promise<any> {
    const access_token = await this.getValidAccessToken();
    const response = await this.data.postUrl(this.institute.Url + "/Naplo/v2/Ertekeles/OsztalyCsoportErtekeles", data, {
      'Authorization': 'Bearer ' + access_token,
    }, "json");
  ...
  async removeDayFromCache(day: Date) {
    ...
    await this.data.removeItem(this.institute.Url + "/Naplo/v2/Orarend/OraLista?datumUtc=" + date.toISOString());
  }

  async removeMulasztasFromCache(tanoraid: number) {
    await this.data.removeItem(this.institute.Url + "/Naplo/v2/Ora/Mulasztas?hash=&tanoraId=" + tanoraid);
  }

  async removeFeljegyzesFromCache(tanoraid: number) {
    await this.data.removeItem(this.institute.Url + "/Naplo/v2/Ora/Feljegyzes?hash=&tanoraId=" + tanoraid);
  }
