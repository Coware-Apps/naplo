import { Component, OnInit, ViewChildren, QueryList, Input, OnChanges, SimpleChanges } from '@angular/core';
import { KretaEnum, Lesson, OsztalyTanuloi, KretaTanuloErtekeles, ErtekelesTipus } from 'src/app/_models';
...
export class ErtekelesComponent implements OnInit, OnChanges {
  ...
  async ngOnInit() {
    ...
    this.ertekelesModLista = (await this.kreta.getNaploEnum("ErtekelesModEnum")).filter(x => x.Nev != 'Na');
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['lesson']) {
      ...
      if (this.lesson && this.lesson.KezdeteUtc)
        this.ertekelesDatum = this.dateHelper.stripTimeFromDate(this.lesson.KezdeteUtc).toISOString();
    }
  }
  ...
  async presentErtekelesModPicker() {
    const picker = await this.picker.create({
      ...
      columns: [
        {
          name: 'ertekelesMod',
          selectedIndex: this.ertekelesMod ? this.ertekelesModLista.findIndex(x => x.Id == this.ertekelesMod.Id) : 0,
          options: this.ertekelesModLista.map(x => { return { value: x.Id, text: x.Nev }; }),
        }
      ]
    });
  ...
  public async save(): Promise<boolean> {
    ...
    if (ertekelesLista.length > 0) {
      try {
        ...
        if (ertekelesResponse && ertekelesResponse[0] && ertekelesResponse[0].Exception && ertekelesResponse[0].Exception.Message) {
          let hibak = "";
          ertekelesResponse[0].Exception.ValidationItems.forEach(x => hibak += x.Id + ": " + x.Message + "<br>");
          await this.error.presentAlert(hibak, ertekelesResponse[0].Exception.Message, "Hiba az értékelések mentésekor");
        ...
        if (e.error) {
          ...
          await this.error.presentAlert("Szerveroldali hiba történt az értékelések mentése során: " + err.Message);
        }
  ...
  public updateAtlag(): void {
    ...
    ertekelesLista.forEach(e => {
      ...
      else if (this.ertekelesTipus == ErtekelesTipus.Szazalek)
        sum += e.Ertekeles.Szazalek;
    });
