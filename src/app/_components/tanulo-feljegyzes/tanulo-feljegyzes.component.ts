...
export class TanuloFeljegyzesComponent implements OnInit {
  ...
  public toggleFeljegyzes(tipus: string) {
    if (tipus == 'hazifeladathiany')
      this.hazifeladathiany = !this.hazifeladathiany;
    if (tipus == 'felszereleshiany')
      this.felszereleshiany = !this.felszereleshiany;
    if (tipus == 'felmentes')
      this.felmentes = !this.felmentes;
    if (tipus == 'dicseret')
      this.dicseret = !this.dicseret;
  }

  public getJsonOutput(): KretaEnum[] {
    let postableFeljegyzesek = [];
    if (this.hazifeladathiany) postableFeljegyzesek.push(this.feljegyzesKodok.find(x => x.Nev == "HaziFeladatHiany"));
    if (this.felszereleshiany) postableFeljegyzesek.push(this.feljegyzesKodok.find(x => x.Nev == "Felszereleshiany"));
    if (this.felmentes) postableFeljegyzesek.push(this.feljegyzesKodok.find(x => x.Nev == "SzakmaiMentessegNemHivatalos"));
    if (this.dicseret) postableFeljegyzesek.push(this.feljegyzesKodok.find(x => x.Nev == "Dicseret"));
