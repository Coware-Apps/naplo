...
export class TanuloErtekelesComponent implements OnInit {
  ...
  changeSzazalek(diff: number) {
    ...
    if (sz + diff <= 100 && sz + diff >= 0)
      this.szazalek = sz + diff;
  }
  ...
  public getJsonOutput(): KretaTanuloErtekeles {
    ...
    return {
      ...
      Ertekeles: {
        OsztalyzatTipus: this.tipus == ErtekelesTipus.Osztalyzat ? this.osztalyzatKodok.find(x => x.Id == this.jegy + 1500) : null,
