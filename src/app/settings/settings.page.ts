...
export class SettingsPage implements OnInit, OnDestroy {
  ...
  changeErtekelesTipus($event) {
    ...
    this.firebase.logEvent("pref_defertekelestipus_change", { newSetting: $event.detail.checked });
  }
        ...
        if (available) {
          ...
          this.safariViewController.show({
            ...
            .subscribe(
              (result: any) => { },
              (error: any) => {
                this.firebase.logError("settings privacy policy modal subscribe error: " + error);
                console.error(error);
              }
            );
